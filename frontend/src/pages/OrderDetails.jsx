...
const OrderDetails = () => {
      ...
      try {
        ...
        setError(
          err.response?.data?.message ||
            "Failed to fetch order details: " + err.message
        );
        toast.error(
          err.response?.data?.message ||
            "Failed to fetch order details: " + err.message,
  ...
  const handleCancelOrder = async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || "Failed to cancel order: " + err.message
      );
      toast.error(
        err.response?.data?.message || "Failed to cancel order: " + err.message,
        { position: "top-right", autoClose: 3000 }
  ...
  const couponDiscount = order.couponId
    ? (order.subTotal + order.vat + order.shippingFee - order.total).toFixed(2)
    : null;
            ...
            <ul className="space-y-3">
              {order.items.map((item) => (
                ...
                >
                  <img
                    src={
                      item.ingredientId.image?.secure_url || "/placeholder.png"
                    }
